...
class LanguageCorrectionTests(unittest.TestCase):
 ...
 def test_negated_employment_cancels_pending_date_before_date_consumption(self):
  staff.load_staff_example(self.store);chat.converse(self.store,self.thread,'Lisa quit');self.assertEqual(self.thread['staffing_pending']['kind'],'end_date');chat.converse(self.store,self.thread,'Actually Lisa did not quit on 2026-09-15.');self.assertIsNone(self.thread['staffing_pending']);self.assertIsNone(self.lisa()['end_date']);chat.converse(self.store,self.thread,'2026-09-16');self.assertIsNone(self.lisa()['end_date'])
 def test_pending_end_date_denial_can_use_pronoun_and_date(self):
  staff.load_staff_example(self.store);chat.converse(self.store,self.thread,'Lisa quit');answer=chat.converse(self.store,self.thread,'No, she is staying. 2026-09-15 was a mistake.');self.assertEqual(answer['kind'],'staffing_clarification');self.assertIsNone(self.lisa()['end_date']);self.assertIsNone(self.thread['staffing_pending'])
 ...
 def test_saved_ended_record_is_not_silently_erased_by_denial(self):
  staff.load_staff_example(self.store);chat.converse(self.store,self.thread,'Lisa quit on 2026-09-15.');self.assertEqual(self.lisa()['end_date'],'2026-09-15');answer=chat.converse(self.store,self.thread,'Lisa did not quit on 2026-09-15.');self.assertIn('currently has a saved end date of 2026-09-15',answer['answer']);self.assertEqual(self.lisa()['end_date'],'2026-09-15')
