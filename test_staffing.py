...
class StaffingTests(unittest.TestCase):
 ...
 def test_quit_effective_date_preserves_past_and_flags_future(self):
  a=self.add('Adam');self.saved_day([(a,540,1020)]);review=ask(self.store,'Adam quit on '+W);self.assertEqual('employment_end',review['navigation']['view']);self.assertEqual(W,review['navigation']['proposed_date']);self.assertIsNone(s.employee(self.store,a['id'])['end_date'])
  s.end_employment(self.store,a['id'],W,'Explicitly reviewed end date');self.assertEqual(W,s.employee(self.store,a['id'])['end_date']);self.assertTrue(s.conflicts(self.store,d.latest(self.store,W)));self.assertTrue(d.latest(self.store,W)['shifts']);self.assertTrue(s.active(s.employee(self.store,a['id']),'2026-09-12'))
